...
import { updateFaceData, geocodeLocation, getCompanySettings, updateCompanySettings } from '../services/authService';
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    geoPos ? Math.round(geoPos.latitude * 100000) : null,
    geoPos ? Math.round(geoPos.longitude * 100000) : null,
  ]);
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">Account Role</p>
              <p className="text-gray-900 font-medium capitalize">
                {user?.role || 'User'} 
                {user?.role === 'admin' ? ' (Screen Sharing Allowed)' : ' (Screen Sharing Blocked)'}
              </p>
            </div>
          </div>
        </div>
        ...
        {/* ── Admin Settings Card ── */}
        {user?.role === 'admin' && (
          <div className="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-2xl shadow-lg p-8 mb-6 border-l-4 border-indigo-500">
            <h2 className="text-xl font-bold text-indigo-900 mb-2 flex items-center gap-2">
              ...
              Global Company Settings (Admin Only)
            </h2>
            <p className="text-sm text-indigo-700 mb-6 font-medium">Changes here apply permanently to ALL current and future employees.</p>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-white p-6 rounded-xl border border-indigo-100 shadow-sm">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Office Hours Per Day</label>
                <input 
                  type="number"
                  step="0.5"
                  min="1"
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Weekly Off Days</label>
                <input 
                  type="text"
            ...
            <div className="mt-5 flex justify-end">
              <button 
                ...
                className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-lg font-medium transition-colors shadow-sm disabled:opacity-50 flex items-center gap-2"
              >
                {isSavingSettings ? (
          ...
          {latestSession?.address && (
            <div className="mt-4 bg-gray-50 rounded-xl p-4 border border-gray-100">
              ...
              <p className="text-gray-800 text-sm font-medium leading-snug">
                {latestSession.address.display_name || "Address not available"}
              </p>
            </div>
          )}
        </div>
